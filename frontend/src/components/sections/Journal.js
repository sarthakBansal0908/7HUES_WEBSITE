        ...
        <div className="grid lg:grid-cols-12 gap-10 items-end mb-16">
          <div className="lg:col-span-4">
            <Reveal>
              ...
              <h2 className="headline text-6xl md:text-7xl text-ink mb-6">{data.title}</h2>
          ...
          <div className="lg:col-span-8 grid sm:grid-cols-3 gap-8">
            {(data.posts || []).map((post, i) => (
              <Reveal key={i} delay={i * 0.1} className={i === 1 ? 'sm:mt-12' : ''}>
                <Link to={`/journal/${post.slug}`} data-testid={`journal-card-${i}`} className="group block">
                  <MaskImage
                    src={post.image}
                    alt={post.title}
                    className="aspect-[3/4] w-full"
                    imgClass="group-hover:scale-105 transition-transform duration-[1400ms]"
                  />
                  <span className="overline text-gold block mt-5">{post.category}</span>
                  <h3 className="font-display uppercase tracking-[0.03em] text-xl mt-2 leading-tight text-ink">
                    {post.title}
                  </h3>
                </Link>
              </Reveal>
